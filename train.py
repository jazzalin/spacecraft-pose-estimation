...
def esa_attitude_loss(att_gt, att_pred):
    ...
    prod = torch.bmm(att_gt.view(bs, 1, 4), att_pred.view(bs, 4, 1)).reshape(bs, 1)
    # loss = 1 - prod ** 2
    loss = 2*torch.acos(torch.abs(prod))
...
def evaluate(val_loader, net, loss_graph=None):
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader):
            ...
            # Attitude conversion: None required
...
if __name__ == "__main__":
    ...
    scheduler_a = torch.optim.lr_scheduler.StepLR(optimizer_a, step_size=40, gamma=0.5)
    ...
    trained_net.to(device)
    trained_net.eval()
    train_loss = evaluate(train_loader, trained_net)
    print("========================= SCORES =========================")
